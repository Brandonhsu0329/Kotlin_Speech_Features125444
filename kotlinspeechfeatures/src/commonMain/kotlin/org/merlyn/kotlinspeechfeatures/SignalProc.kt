...
//    fun magspec123123test(): Array<Complex> {
//        val frames: FloatArray = floatArrayOf(4.0f, 5.0f, 6.0f)
//        val result = fft.rfft(frames,512)
//        absOutput.addAll(result.subList(0, result.size / 2 - 1).map {
//                    modul(it.re().toFloat(), it.im().toFloat())
//                })
//        return result
//    }
    fun magspec123123(frames: Array<FloatArray>, nfft: Int): List<Complex> {

//        val frames: Array<FloatArray> = arrayOf(
//            floatArrayOf(1.0f, 2.0f, 3.0f),
//            floatArrayOf(4.0f, 5.0f, 6.0f))
//        val framesss: FloatArray = floatArrayOf(4.0f, 5.0f, 6.0f)
        // 獲取音訊帧的寬度
        ...
        runBlocking {
            ...
            frames.asyncForEachIndexed { index, frame ->
                ...
                val fft = FloatFFT_1D(input.size.toLong())
                val result = fft.realForward(input)
                // 下方式原本的
//                val result = fft.rfft(input, nfft).toList()
...
//        val input = addZeros(framesss,(nfft - frameWidth))
//        val input = frames + FloatArray(nfft - frameWidth)
//        val input = frames[0] + FloatArray(nfft - frameWidth)
//        val input = frames[0] + FloatArray(nfft-frameWidth)
//        return nfft - frameWidth
//        return input.size
        val framesss: FloatArray = floatArrayOf(4.0f, 5.0f, 6.0f)
        val input = framesss + FloatArray(4- 3)
        val result = fft.rfft(input, nfft).toList()
        return result
    }
    ...
    fun powspec123123(frames: Array<FloatArray>, nfft: Int): List<Complex>{
        val fftOut = nfft / 2 + 1
